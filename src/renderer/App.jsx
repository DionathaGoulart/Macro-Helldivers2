...
import { hasExclusiveConflict, normalizeText } from './lib/build'

// Chunk separado: arrasta equipment.json + statsMap.json (~100 KB) e só é buscado
// quando o usuário abre a aba de builds pela primeira vez
...
function App() {
  ...
  // A aba de builds só é montada na primeira visita e nunca desmontada: assim o chunk
  // não entra no boot e a build gerada não some ao trocar de aba
  const [buildMounted, setBuildMounted] = useState(false)
  ...
  useEffect(() => {
    if (activeTab === 'build') setBuildMounted(true)
  }, [activeTab])
  ...
  const flashBackupStatus = (status) => {
    ...
    backupTimerRef.current = setTimeout(() => setBackupStatus(null), 2500)
  }

  useEffect(() => () => clearTimeout(backupTimerRef.current), [])
  ...
  return (
    ...
    <div className={`h-screen flex flex-col ${
      ...
      <main className="flex-1 overflow-y-auto scrollbar-hd relative flex flex-col">
        ...
        {/* display:contents mantém o layout idêntico ao da versão inline; a aba
            fica montada depois da primeira visita pra não perder a build gerada */}
        {buildMounted && (
          <div style={{ display: activeTab === 'build' ? 'contents' : 'none' }}>
            <Suspense fallback={
              <p className="text-center text-[10px] text-slate-600 uppercase tracking-widest py-12">
                {t.build.metaLoading}
              </p>
            }>
              <BuildTab
                ...
                loadouts={loadouts}
                persistLoadouts={persistLoadouts}
                isOverlay={isOverlay}
              />
            </Suspense>
          </div>
